...
class HospitalPatient(models.Model):
    ...
    def print_report(self):
        return self.env.ref('timhergty_hospital.report_print_patient_card').report_action(self)
    ...
    cause_of_death = fields.Char(string="ause of Death")
    receivable = fields.Float(string="Receivable", readonly=True)
    current_insurance_id = fields.Many2one('hospital.insurance', string="Insurance")
    ...
    primary_care_physician_id = fields.Many2one('hospital.physician', string="Primary Care Doctor")
    patient_status = fields.Char(string="Hospitalization Status", readonly=True)
    patient_disease_ids = fields.One2many('hospital.patient.disease', 'patient_id')
    patient_psc_ids = fields.One2many('hospital.patient.psc', 'patient_id')
    ...
    smoking_number = fields.Integer(string="Cigarettes a day")
    ...
    cage_ids = fields.One2many('hospital.patient.cage', 'patient_id')
    ...
    menstrual_history_ids = fields.One2many('hospital.patient.menstrual.history', 'patient_id')
    ...
    mammography_history_ids = fields.One2many('hospital.patient.mammography.history', 'patient_id')
    pap_history_ids = fields.One2many('hospital.patient.pap.history', 'patient_id')
    colposcopy_history_ids = fields.One2many('hospital.patient.colposcopy.history', 'patient_id')
    ...
    pregnancy_history_ids = fields.One2many('hospital.patient.pregnancy', 'patient_id')
    family_history_ids = fields.Many2many('hospital.family.disease', string="Family Disease Lines")
    perinatal_ids = fields.Many2many('hospital.perinatal')
    ...
    lab_test_ids = fields.One2many('hospital.patient.lab.test', 'patient_id')
    fertile = fields.Boolean('Fertile')
    ...
    hospital_vaccination_ids = fields.One2many('hospital.vaccination', 'hospital_patient_vaccines_id')
    hospital_appointments_ids = fields.One2many('hospital.appointment', 'patient_id', string='Appointments')
    lastname = fields.Char('Last Name')
    report_date = fields.Date('Date', default=datetime.today().date())
    medication_ids = fields.One2many('hospital.patient.medication1', 'hospital_patient_medication_id')
    ...
    def create(self, val):
        ...
        patient_id = self.env['ir.sequence'].next_by_code('hospital.patient')
        if patient_id:
            val.update({
                'name': patient_id,
            })
        result = super(HospitalPatient, self).create(val)
        return result
