...
class HospitalPatientOvulation(models.Model):
    _name = 'hospital.patient.ovulation'
    _description = 'Hospital Patient Ovulation'
    _rec_name = 'hospital_patient_id'

    patient_id = fields.Many2one('res.partner', domain=[('is_patient', '=', True)], string="Patient")
    hospital_patient_id = fields.Many2one('hospital.patient', string="Patient", required=True)
    start_ovulation = fields.Datetime(string="Start Ovulation")
    physician_partner_id = fields.Many2one('res.partner', domain=[('is_doctor', '=', True)], string="Doctor")
    end_ovulation = fields.Datetime(string="End of Ovulation")
    ovulation_type = fields.Selection([
        ('a', 'Ambulatory'),
        ('e', 'Emergency'),
        ('i', 'Inpatient'),
    ...
    tag = fields.Integer(
        string='Last TAGs',
        help='Triacylglycerol(triglycerides) level. Can be approximated'
    )
    ...
    ovulation_summary = fields.Text(string='Ovulation Summary')
    ...
    diagnosis_id = fields.Many2one('hospital.pathology', 'Presumptive Diagnosis')
    ldl = fields.Integer(
        string='Last LDL',
    ...
    urgency = fields.Selection([('a', 'Normal'), ('b', 'Urgent'), ('c', 'Hospital Emergency')], string='Urgency')
    systolic = fields.Integer('Systolic Pressure')
    diastolic = fields.Integer('Diastolic Pressure')
    respiratory_rate = fields.Integer('Respiratory Rate')
    signs_and_symptoms_ids = fields.One2many('hospital.signs.and.symptoms', 'patient_ovulation_id',
                                             'Signs and Symptoms')
    ...
    hospital_appointment_date_id = fields.Many2one('hospital.appointment', 'Appointment Date')
    next_appointment_id = fields.Many2one('hospital.appointment', 'Next Appointment')
    derived_from_physician_id = fields.Many2one('hospital.physician', 'Derived from Doctor')
    derived_to_physician_id = fields.Many2one('hospital.physician', 'Derived to Doctor')
    secondary_conditions_ids = fields.One2many('hospital.secondary_condition', 'patient_ovulation_id',
                                               'Secondary Conditions')
    diagnostic_hypothesis_ids = fields.One2many('hospital.diagnostic_hypothesis', 'patient_ovulation_id', 'Procedures')
    procedure_ids = fields.One2many('hospital.directions', 'patient_ovulation_id', 'Procedures')
