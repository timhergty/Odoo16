...
class HospitalAppointment(models.Model):
    ...
    inpatient_registration_id = fields.Many2one('hospital.inpatient.registration', string="Inpatient Registration")
    ...
    doctor_id = fields.Many2one('hospital.physician', 'Physician', required=True)
    ...
    hospital_patient_psc_ids = fields.Many2many('hospital.patient.psc', string='Pediatrics Symptoms Checklist')
    hospital_prescription_order_ids = fields.One2many('hospital.prescription.order', 'appointment_id',
                                                      string='Prescription')
    ...
    def onchange_patient(self):
        if not self.inpatient_registration_id:
            self.patient_id = ""
        inpatient_obj = self.env['hospital.inpatient.registration'].browse(self.inpatient_registration_id.id)
    ...
    def print_prescription(self):
        return self.env.ref('timhergty_hospital.report_print_prescription').report_action(self)
