    ...
    def create_lab_test(self):
        ...
        for browse_record in self:
            ...
            if res_ids:
                ...
                action = self.env.ref('timhergty_hospital.action_hospital_lab_form')
                list_view_id = imd.sudo()._xmlid_to_res_id('timhergty_hospital.hospital_lab_tree_view')
                form_view_id = imd.sudo()._xmlid_to_res_id('timhergty_hospital.hospital_lab_form_view')
                result = {
                    'name': action.name,
                    'help': action.help,
                    'type': action.type,
